...
define(function(require, exports, module){
	//
	var VerticalView = require('src/schema/ui/biz/media/tab/verticalcontent');
	...
	var View = Backbone.View.extend({
		initialize: function(){
			...
			var verticalImage = _.find(this.model.layout.children(), function(field){
				return (field.get('type') === 'vertical-image');
			});
			if(verticalImage){
				this.content = new VerticalView({model: {field: verticalImage, parent: this}});
			}
		...
		renderTab: function(){
			var isMust = (this.verticalImage.get('must') === true);
			if(isMust){
				this.$tab.append($('<span class="must">').text('*'));
			}

			var title = this.verticalImage.get('title');
