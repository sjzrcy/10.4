...
Rectangle {
    ...
    property int imagesInOnePage:6
    ...
    property bool supportSixthImg:false
    ...
    function supportSixthImage(enable)
    {
        supportSixthImg = enable;
    }
    ...
    {
        if(index < imagesInOnePage && column == 0)
        {
            showingModel.set(index, newElement);
        }
        else if(index >= imagesInOnePage && column == 0)
        {
            hiddenModel.set(index - imagesInOnePage, newElement);
        }
        else if(index < imagesInOnePage && column == 1)
        {
            hiddenModel.set(index - imagesInOnePage, newElement);
        }
        else if(index >= imagesInOnePage && column == 1)
        {
            showingModel.set(index - imagesInOnePage, newElement);
        }
    ...
    {
        showImagesWidget.setCurrentIndex(imagesInOnePage*column + list.currentIndex);
    }
    ...
    function swape()
    {
        ...
        var i =0;
        //第六张竖图不参与swap移动图片
        for(i; i<showingModel.count-1; i++)
        {
    ...
    Rectangle {
        ...
        ListView {
            ...
            MouseArea {
                ...
                {
                    if (index < imagesInOnePage-1 && newIndex < imagesInOnePage-1 && index >=0 && newIndex >=0 && loc.currentId != -1 && index != newIndex)
                    {
                        if(showImagesWidget.isValidSwape(column*imagesInOnePage+newIndex, column*imagesInOnePage+index))
                        {
                            showImagesWidget.swapeImages(column*imagesInOnePage+newIndex, column*imagesInOnePage+index)
    ...
    Image {
        ...
        Rectangle {
            ...
            Text {
                text: (list.currentIndex==5)?"<div>竖图规范：</div><div>宽：高=2：3</div><div>且最小长度为480px</div>":"暂无图片"
                anchors {
